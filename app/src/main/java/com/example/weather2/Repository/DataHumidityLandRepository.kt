...
class DataHumidityLandRepository ( ) {

    private val database = FirebaseDatabase.getInstance()
    private val humidityRef = database.getReference("data_humidity_land")

    companion object {
        private const val TAG = "HumidityRepository"
    }

    fun getDataHumidityLandList(): Flow<List<data_humidity_land>> = callbackFlow {
        Log.d(TAG, "Starting to observe humidity list")

        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                try {
                    val humidityList = mutableListOf<data_humidity_land>()

                    for (childSnapshot in snapshot.children) {
                        val humidity = childSnapshot.getValue(data_humidity_land::class.java)
                        humidity?.let {
                            humidityList.add(it)
                            Log.d(TAG, "Added humidity: ${it.humidity_land}% at ${it.time}")
                        }
                    }
        ...
        Log.e(TAG, "Flow error in getDataHumidityLandList", e)
        emit(emptyList())
    }.onStart {
        Log.d(TAG, "Flow started for humidity list")
    }

    fun getLatestDataHumidityLand(): Flow<data_humidity_land?> = callbackFlow {
        ...
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                try {
                    var latestHumidity: data_humidity_land? = null
                    ...
                    Log.d(TAG, "Latest humidity: ${latestHumidity?.humidity_land}% at ${latestHumidity?.time}")
        ...
        Log.e(TAG, "Flow error in getLatestDataHumidityLand", e)
        emit(null)
    }
    ...
    suspend fun deleteDataByDateRange(startDate: String, endDate: String): Boolean {
        return try {
            Log.d(TAG, "Bắt đầu xóa dữ liệu từ $startDate đến $endDate")
            ...
            Log.d(TAG, "Xóa thành công $deletedCount records")
            true
    ...
    suspend fun getDataByDateRange(startDate: String, endDate: String): List<data_humidity_land> {
        return try {
            Log.d(TAG, "Lấy dữ liệu từ $startDate đến $endDate")

            val snapshot = humidityRef.get().await()
            val filteredList = mutableListOf<data_humidity_land>()
            ...
            Log.d(TAG, "Tìm thấy ${sortedList.size} records trong khoảng thời gian")
